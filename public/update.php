...
function limpiarCampo(string $campo): string
{
    return htmlspecialchars(trim($campo));
}

if (isset($_POST['btn'])) {
    $nombre = ucfirst(limpiarCampo($_POST['nombre']));
    $descripcion = ucfirst(limpiarCampo($_POST['descripcion']));
    $precio = (float)trim($_POST['precio']);
    $stock = (int)trim($_POST['stock']);

    $errores = false;
    if (Utils::hayErrorEnCampo('Nombre', $nombre, 3, $id)) {
        $errores = true;
    }
    ...
    if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
        if (Utils::errorEnTipoImagen($_FILES['imagen']['type'])) {
            ...
            if (!move_uploaded_file($_FILES['imagen']['tmp_name'], $nombreI)) {
                ...
                if (basename($articulo->getImagen()) != 'default.png') {
                    unlink("." . $articulo->getImagen());
                }
            }
        }
    } else {
    ...
    (new Articulos)->setNombre($nombre)
        ...
        ->setImagen($imagen)
        ->update($id);
    $_SESSION['mensaje'] = "Artículo editado con éxito";
    header("Location:inicio.php");
            ...
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF'] . "?id=$id"; ?>" enctype="multipart/form-data">
                <div class="mb-6">
                    <label for="nombre" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Nombre</label>
                    <input type="text" name="nombre" id="nombre" value="<?php echo $articulo->getNombre() ?>"
                           placeholder="Nombre..."
                ...
                <div class="mb-6">
                    <label for="precio" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Precio (€)</label>
                    <input type="number" id="precio" name="precio" value="<?php echo $articulo->getPrecio() ?>"
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                           step="0.01" max="9999,99" min="0">
                    <?php
                ...
                <div class="mb-6">
                    <label for="stock" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Stock</label>
                    <input type="number" id="stock" name="stock" value="<?php echo $articulo->getStock() ?>"
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                           step="1" min="0"/>
                    <?php
                ...
                <div class="mb-6">
                    <div class="flex w-full">
                        ...
                        <div class="w-1/2">
                            <img src="<?php echo "." . $articulo->getImagen() ?>"
                                 class="h-72 rounded w-full object-cover border-4 border-black" id="img" alt="">
                        </div>
                    </div>
